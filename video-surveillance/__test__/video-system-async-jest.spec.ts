import { VideoSystemController } from '../src/VideoSystemControllerAsync';
...
describe('Video System Controller', () => {
  ...
  it('should check motion sensor state once per second', async () => {
    const sensorSpy = jest.spyOn(sensor, 'isDetectingMotion');
    controller = new VideoSystemController(sensor, recorder);
    const recordTimeInSeconds = 3;

    await controller.start(recordTimeInSeconds);
